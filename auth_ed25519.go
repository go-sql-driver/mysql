...
// For Ed25519 authentication, we simply return the packet as is since the server
// will verify the signature we sent in InitAuth.
func (p *ClientEd25519Plugin) ProcessAuthResponse(packet []byte, authData []byte, mc *mysqlConn) ([]byte, error) {
	return packet, nil
}
